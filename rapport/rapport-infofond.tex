...
\end{figure}
Le problème de \textbf{Job Shop Scheduling with Sequence-Dependent Setup} est NP-dur \todo{footnote}. Notre problème de gestion d'horaires est en fait un problème de \textbf{Job Shop Scheduling with Sequence-Dependent Setup}.
On pose les examens comme étant des jobs, les salles comme étant des ressources allouables aux jobs, la durée d'un examen comme étant la durée d'un job et on garde les périodes de temps. Dans une salle (ressource), il ne peut pas y avoir deux examens (jobs) en même temps. Chaque examen (job) doit se dérouler exactement une fois et avec exactement une salle (ressource).
Les examens que passent un étudiant et ceux que donnent un professeur sont simplement des contraintes empêchant certains examens (jobs) de se dérouler en même temps. 
